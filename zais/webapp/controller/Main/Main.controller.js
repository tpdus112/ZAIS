sap.ui.define(
  ...
  (Controller, MessageToast) => {
    ...
    return Controller.extend("zais.scm.zais.controller.main.Main", {
      onInit() {
        // 대시보드 모델 등록 확인
        const oComponent = this.getOwnerComponent();
        if (!this.getView().getModel("dashboard")) {
      ...
      _restoreLaunchpadHeader() {
        ...
        if (typeof sap !== "undefined" && sap.ushell && sap.ushell.Container) {
          try {
            ...
            if (oRenderer) {
              if (typeof oRenderer.setHeaderVisibility === "function") {
                oRenderer.setHeaderVisibility(true, false);
              }
              if (typeof oRenderer.showShellHeader === "function") {
                oRenderer.showShellHeader(true);
              }
              if (typeof oRenderer.setLetterboxMode === "function") {
                oRenderer.setLetterboxMode(false);
              }
            }
          } catch (e) {
            console.log("FLP Renderer restore in Main:", e);
          }
        }
      },
