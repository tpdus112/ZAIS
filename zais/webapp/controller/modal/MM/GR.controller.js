sap.ui.define(
  ...
  (Controller, JSONModel, MessageToast, MMDataService) => {
    ...
    return Controller.extend("zais.scm.zais.controller.modal.MM.GR", {
      onInit() {
      ...
      _loadData() {
        ...
        if (oDashboardModel) {
          const aCached = oDashboardModel.getProperty("/grList");
        ...
        MMDataService.loadGoodsReceipts(oComponent, oDashboardModel)
          .then((aList) => {
      ...
      _applyFiltersAndPaging() {
        ...
        const aFiltered = aList.filter((oItem) => {
          if (sKeyword) {
            const bMatchDoc = (oItem.materialDoc || "")
              .toLowerCase()
              .includes(sKeyword);
            ...
            if (!bMatchDoc && !bMatchCode && !bMatchName) {
              return false;
            }
          }
          ...
          if (sStatusKey === "GR_CANCELLED" && oItem.status !== "입고 취소") {
            return false;
          }

          if (sDateFrom && oItem.postingDate < sDateFrom) {
            return false;
          }
          if (sDateTo && oItem.postingDate > sDateTo) {
            return false;
          }
