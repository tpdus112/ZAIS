sap.ui.define(
  ...
  (Controller, JSONModel, MessageToast, MMDataService) => {
    ...
    return Controller.extend("zais.scm.zais.controller.modal.MM.PR", {
      onInit() {
      ...
      _loadData() {
        ...
        if (oDashboardModel) {
          const aCached = oDashboardModel.getProperty("/prList");
        ...
        MMDataService.loadPurchaseRequests(oComponent, oDashboardModel)
          .then((aList) => {
      ...
      _applyFiltersAndPaging() {
        ...
        const aFiltered = aList.filter((oItem) => {
          ...
          if (sStatusKey === "PO_CREATED" && oItem.status !== "PO 생성") {
            return false;
          }
          if (sStatusKey === "PR_REQUESTED" && oItem.status !== "구매요청") {
            return false;
          }
          ...
          if (sDateFrom && oItem.reqDate < sDateFrom) {
            return false;
          }
          if (sDateTo && oItem.reqDate > sDateTo) {
            return false;
          }
